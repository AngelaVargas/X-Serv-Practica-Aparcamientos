...
def login_form(request):
    ...
    form += 'Nombre de usuario<br><input type="text" name="Usuario"><br>'
    form += 'Contraseña<br><input type="password" name="Password"><br>'
    form += '<br><input type="submit" value="Entrar"></form>'
    return form
...
def loginuser(request):
	...
	if user is not None:
		...
		Error = "Este usuario no está registrado en la web. Por favor, introduzca un usuario o contraseña válidos"
...
def pie_pagina():
	...
	foot += '<a href=' + url + '>Descripción de los datos</a>'
...
def lista_comentarios():
	...
	for i in lista_ordenada:
		...
		if len(comentarios) != 0:
			Response += "<a href=" + i.content_url + ">" + i.nombre + "<br></a>"
...
def lista_aparcamientos():
	...
	for aparcamiento in aparcamientos:
		...
		lista += '<p>' + nombre_aparcamiento + '<a href="' + url_aparcamiento + '">	⇾ Más información</a></p>'
...
def aparcamientos_seleccionados(user,request):
	...
	try:
		...
		for i in seleccionados:
			lista += "<br>Fecha de selección: " + str(i.fecha_seleccion)
...
def pagina_principal(request):
	...
	if request.user.is_authenticated():				#He recibido un login de un usuario
		...
		form = 'Bienvenido ' + username
		form += '<br><br><a href="http://localhost:1234/logout" > Logout </a>'
	...
	if request.method == 'POST':
		...
		if key == 'Accesible':
			...
			if value == '1':
				...
				for i in lista_accesibles:
					...
					lista += "<p>" + nombre_aparcamiento + "</p><li><a href=" + url_aparcamiento + ">" + url_aparcamiento + "</a></li>"
			else:
				lista = '<a href="http://localhost:1234/" > Volver </a>'
				aparcamientos = Aparcamiento.objects.all()
				for aparcamiento in aparcamientos:
					nombre_aparcamiento = aparcamiento.nombre
					url_aparcamiento = aparcamiento.entidad
					lista += '<p>' + nombre_aparcamiento + '<a href="aparcamientos/' + url_aparcamiento + '">	⇾ Más información</a></p>'
				value = 1

			accesible = accesibles(value)
			c = Context({'login': form, 'footer': foot, 'list_users':lista, 'accesible': accesible})
		...
		c = Context({'login': form, 'footer': foot, 'list':list_coments, 'list_users':users, 'accesible': accesible})
...
def usuarios(request, peticion):
	...
	form += '<br>Introduzca un título nuevo a su página personal<br><input type="text" name="Titulo">'
	form += '<input type="submit" value=" Actualizar"></form>'
	...
	if request.user.is_authenticated():
		...
		if peticion != username:
			...
			c = Context({'lista_selecc':lista_seleccionados, 'seleccionados':seleccionados, 'lista': lista, 'form': form, 'css':css, 'titulo': titulo_pagina, 'footer':foot})
	else:
		template = get_template("public.html")
		titulo_pagina = "Página pública de " + peticion + "<br><br>"
...
def aparcamientos_id(request, recurso):
	...
	try:
		...
		if request.user.is_authenticated():
			...
			form = '<form action="" method="POST">'
			form += '<br><br>Puede introducir un comentario si lo desea ' + str(request.user) + '<br><input type="text" name="Comentario"><br>'
			form += '<br><input type="submit" value="Enviar"></form>'
			Response += form #Sólo si está logueado
